...
describe Him::Model::Paths do
  context "building request paths" do
    context "simple model" do
      ...
      describe "#request_path" do
        ...
        it "raises exceptions when building a path without required custom variables" do
          Foo::User.collection_path "/organizations/:organization_id/utilisateurs"
          expect { Foo::User.new(id: "foo").request_path }.to raise_error(Him::Errors::PathError, "Missing :_organization_id parameter to build the request path. Path is `/organizations/:organization_id/utilisateurs/:id`. Parameters are `{:id=>\"foo\"}`.")
        end
    ...
    context "simple model with multiple words" do
      ...
      describe "#request_path" do
        ...
        it "raises exceptions when building a path without required custom variables" do
          Foo::AdminUser.collection_path "/organizations/:organization_id/users"
          expect { Foo::AdminUser.new(id: "foo").request_path }.to raise_error(Him::Errors::PathError, "Missing :_organization_id parameter to build the request path. Path is `/organizations/:organization_id/users/:id`. Parameters are `{:id=>\"foo\"}`.")
        end
  ...
  context "making path HTTP requests" do
    before do
      Him::API.setup url: "https://example.com/api/" do |builder|
        builder.use Him::Middleware::FirstLevelParseJSON
        builder.use Faraday::Request::UrlEncoded
        builder.adapter :test do |stub|
